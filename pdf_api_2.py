...
#     result["total_subheadings"] = str(sub_headings)
#     result["text_sizes"] = text_sizes
...
def get_stats_of_file(curr_file_path):
    ...
    # result['text'] = "The User class represents registered users on the system. It contains attributes such as user ID,email, password, and functionalities, which represent the list of functionalities associated with the user. The User class has methods such as register(), login(), uploadPDF(), selectCriteria(),submitReport(), and shareReport().The PDF File class represents a PDF file in the system. It has attributes such as pdfId and filename. The class provides methods like uploadFile(), validateFile(), and processFile().The Accessibility Criteria class represents the criteria used for evaluating accessibility. It has attributes such as criteria, name, and description. The class provides the functionality displayFile(). The Evaluate Accessibility class represents the evaluation process for accessibility. It has attributes like pdfFile and selectedCriteria. The class provides methods such as evaluation(),calculateScore(), generateReport(), giveAlerts(), and giveSuggestions().The Accessibility Report class represents the generated report for accessibility evaluation. It has attributes like rating score, pdf evaluation, and criteria. The class provides functionalities such as displayScore(), displayReport(), displayLink(), and displayAlerts()."
    result["total_pdf_text"] = final_total_pdf_text
    sums = final_total_pdf_text.split('.')
    final_summary = ""
    ...
    # pdf_path = curr_file_path
    # image_folder = os.getcwd()
    # extract_images_from_pdf(pdf_path, image_folder)

    # summary = summarize(final_total_pdf_text, 0.05)
    ...
    # tables_data = []
    # tables = tabula.read_pdf(curr_file_path, pages='all')
